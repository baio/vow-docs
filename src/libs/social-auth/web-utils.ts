...
import { OAuth2AuthenticateOptions } from './auth.models';
...
export class WebUtils {
  ...
  static getUrlParams(url: string): { [x: string]: string } | undefined {
    ...
    if (hashIndex > -1 && queryIndex === -1) {
      ...
      paramsIndex =
        hashIndex > -1 && hashIndex < queryIndex ? hashIndex : queryIndex;
    }
  ...
  static async buildWebOptions(
    ...
    const mapHelper = this.getOverwritableValue<{ [key: string]: string }>(
      configOptions,
  ...
  static deriveChallenge(codeVerifier: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      crypto.subtle.digest('SHA-256', this.toUint8Array(codeVerifier)).then(
        (arrayBuffer) =>
          resolve(this.toBase64Url(this.toBase64(new Uint8Array(arrayBuffer)))),
        (error) => reject(error)
      );
    });
  }
